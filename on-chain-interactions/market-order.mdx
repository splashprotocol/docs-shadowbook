...
<Tabs>
  ...
  <Tab title="Sell (token → ADA)">
    ...
    ```typescript
    price: [1, inputAmount] // e.g. [1, 100] for 100 tokens input
    ```
  </Tab>
</Tabs>
...
See [datum building](/on-chain-interactions/datum-building) for field reference, address serialization, and the full beacon algorithm.
...
<Steps>
  ...
  <Step title="Build datum">
    Set `price: [1, inputAmount]` and all other fields except beacon. See [datum building](/on-chain-interactions/datum-building).
  </Step>
